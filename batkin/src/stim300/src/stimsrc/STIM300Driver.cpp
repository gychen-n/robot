...
bool DecodeSTIMDatagram(const unsigned char (&frame)[L], STIMData &dat)
{
	if(IsDateTrue(frame))
	{
		...
		if( (dat.gyro_ST == 0x00)&&(dat.acc_ST == 0x00)&&(dat.incli_ST == 0x00) )
			return true;
		else
			return false;
	}
...
{
	return PortIint(usb, 921600);
}

bool stim_log_display(stimdata &stimdat)
{
	static stimlogger logger;
