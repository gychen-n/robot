...
void KeywordCB(const std_msgs::String::ConstPtr &msg)
{
	...
	if (nState == STATE_PHOTO)
	{
		...
		{ //要识别的人名个数
			bGotoExit = true;
			{
				...
				sprintf(order, "sed -i 's/person0/%s/g' ~/face/face.py", per[1]); //格式化命令，替换
				n = system(order);
				sprintf(order, "sed -i 's/person1/%s/g' ~/face/face.py", per[2]); //格式化命令，替换
				n = system(order);
				sprintf(order, "sed -i 's/person2/%s/g' ~/face/face.py", per[3]); //格式化命令，替换
				n = system(order);

				//修改输出pdf文件代码
				sprintf(order, "sed -i 's/person0/%s/g' ~/face/turn_pdf.sh", per[1]); //格式化命令，替换
				n = system(order);
				sprintf(order, "sed -i 's/person1/%s/g' ~/face/turn_pdf.sh", per[2]); //格式化命令，替换
				n = system(order);
				sprintf(order, "sed -i 's/person2/%s/g' ~/face/turn_pdf.sh", per[3]); //格式化命令，替换
				n = system(order);
	...
	if (nState == STATE_PHOTOGRAPH)
	{

		int n, o;
		char order[100];
		sprintf(order, "sed -i 's/%s/%s/g' ~/face/photograph.py", per[pernum - 2], per[pernum - 1]); //格式化命令，替换
		n = system(order);
		Speak("Please don't move");
		o = system("sh ~/face/photo.sh"); //拍照
		sleep(1);
		//nPersonCount=3;
		nPersonCount++;
		if (nPersonCount >= 3)
		...
		else
		{
			...
			if (nPersonCount == 1)
				n = system("sh /home/robot/left.sh"); //左转前进右转
			if (nPersonCount == 2)
				n = system("sh /home/robot/left.sh "); //右转前进左转
